...
def main():
  info = subprocess.check_output([mega_cmd, '-AdpAllInfo', '-aAll', '-nolog']).decode('utf-8').splitlines()
  pdlist = subprocess.check_output([mega_cmd, '-PdList', '-aAll', '-nolog']).decode('utf-8').splitlines()
  out = {}
  adapter = None
...
#  print json.dumps(out, indent=2, sort_keys=True)
  ...
  for k,v in out.items():
    export_file.write("# HELP " + k + " " + v['help'] + "\n")
    export_file.write("# TYPE " + k + " " + v['type'] + "\n")
    #print("# HELP " + k + " " + v['help'])
    #print("# TYPE " + k + " " + v['type'])
    for m in v['metrics']:
      export_file.write( str(k) + '{' + ', '.join([ "{}=\"{}\"".format(str(l),str(m['labels'][l])) for l in sorted(m['labels']) ]) + '} ' +  str(m['val']) + "\n" )
  export_file.close()

if __name__ == "__main__":
  subprocess.call("megaclisas-status")
  main()
